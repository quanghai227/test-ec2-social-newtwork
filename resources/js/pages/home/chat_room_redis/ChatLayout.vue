...
export default {
    ...
    created() {
        console.log('credenttials', this.echoCredentials)
        this.loadMessage()
        let chanel = Echo.channel('laravel_database_chatroom')
            .listen('MessagePostedRedis', (data) => {
    ...
    methods: {
        async loadMessage() {
            try {
                const response = await axios.get("api/messages", {headers: { Authorization: 'Bearer ' + this.getToken }})
        ...
        async getUsersOnline() {
            try {
                const response = await axios.get(`${window.location.protocol}//${window.location.hostname}:6001/apps/${this.echoCredentials.appId}/channels/laravel_database_chatroom?auth_key=${this.echoCredentials.key}`, {headers: { Authorization: 'Bearer ' + this.getToken }})
